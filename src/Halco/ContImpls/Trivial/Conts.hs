...
updateAttrs :: Scheme -> OutAttrs -> Set Attr
updateAttrs s = \case
...
instance Conts.OutCont Scheme OutCont where
  toScheme = Conts.update1 empty

instance Conts.OutCont1 Scheme OutCont where
  update1 s o = Scheme { attrs = updateAttrs s $ attrsO o
                      , props = propsO o <> (props s \\ propsO' o) }

instance Conts.OutCont2 Scheme OutCont where
